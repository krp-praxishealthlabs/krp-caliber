...
function buildInsightsData(stats: ROIStats) {
  ...
  const failureRateImprovement =
    hasSufficientCohorts &&
    failureRateWith !== null &&
    failureRateWithout !== null &&
    failureRateWithout > 0
...
function displayColdStart(score: ScoreResult) {
  ...
  if (!hooksInstalled) {
    ...
    console.log(
      chalk.dim('  Session learning captures patterns from your AI coding sessions — what'),
    );
    console.log(
      chalk.dim('  fails, what works, corrections you make — so your agents improve over time.\n'),
    );
    console.log(
      chalk.dim('  Run ') +
        chalk.cyan(`${displayCaliberName()} learn install`) +
        chalk.dim(' to enable.'),
    );
  } else {
...
function displayEarlyData(data: ReturnType<typeof buildInsightsData>, score: ScoreResult) {
  ...
  if (data.totalWasteTokens > 0) {
    console.log(
      `  Waste captured:         ${chalk.cyan(data.totalWasteTokens.toLocaleString())} tokens`,
    );
  }

  if (data.failureRateImprovement !== null && data.failureRateImprovement > 0) {
    console.log(
      `  Failure rate trend:     ${chalk.green(`${data.failureRateImprovement}% fewer`)} failures with learnings ${chalk.dim('(early signal)')}`,
    );
  } else if (data.totalSessions > 0 && data.failureRateImprovement === null) {
    console.log(
      `  Failure rate trend:     ${chalk.dim('collecting data (need 3+ sessions in each group)')}`,
    );
  }

  if (data.taskSuccessRate !== null) {
    console.log(
      `  Task success rate:      ${chalk.cyan(`${data.taskSuccessRate}%`)} ${chalk.dim(`(${data.taskCount} tasks)`)}`,
    );
  }
...
function displayFullInsights(data: ReturnType<typeof buildInsightsData>, score: ScoreResult) {
  ...
  if (data.taskSuccessRate !== null) {
    const color =
      data.taskSuccessRate >= 80
        ? chalk.green
        : data.taskSuccessRate >= 60
          ? chalk.yellow
          : chalk.red;
    console.log(
    ...
    if (data.taskCorrectionCount > 0) {
      console.log(
        `    Corrections needed:   ${chalk.yellow(String(data.taskCorrectionCount))} tasks required user correction`,
      );
    }
  }
  console.log(`    Sessions tracked:     ${chalk.cyan(String(data.totalSessions))}`);
  ...
  if (data.failureRateWith !== null && data.failureRateWithout !== null) {
    console.log(
      `    Failure rate:         ${chalk.red(data.failureRateWithout.toFixed(1))}/session ${chalk.dim('\u2192')} ${chalk.green(data.failureRateWith.toFixed(1))}/session with learnings`,
    );
    if (data.failureRateImprovement !== null && data.failureRateImprovement > 0) {
      console.log(
        `    Improvement:          ${chalk.green(`${data.failureRateImprovement}%`)} fewer failures`,
      );
    } else if (data.failureRateImprovement === null) {
      console.log(
        `    Improvement:          ${chalk.dim('collecting data (need 3+ sessions in each group)')}`,
      );
    }
  }
  ...
  if (data.totalWasteTokens > 0 || data.estimatedSavingsTokens > 0) {
    ...
    if (data.estimatedSavingsTokens > 0) {
      console.log(
        `    Estimated savings:    ~${chalk.green(data.estimatedSavingsTokens.toLocaleString())} tokens`,
      );
    }
    if (data.estimatedSavingsSeconds > 0) {
      console.log(
        `    Time saved:           ~${chalk.green(formatDuration(data.estimatedSavingsSeconds))}`,
      );
    }
  }
  ...
  if (trend) {
    ...
    console.log(
      `    Trend:                ${trendColor(`${arrow} ${sign}${trend.delta} pts`)} ${chalk.dim(`over ${trend.entries} checks`)}`,
    );
  }
  console.log('');
}
      ...
      JSON.stringify(
        {
          ...
          tier:
            data.totalSessions === 0
              ? 'cold-start'
              : data.totalSessions < MIN_SESSIONS_FULL
                ? 'early'
                : 'full',
